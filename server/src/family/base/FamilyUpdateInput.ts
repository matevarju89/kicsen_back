/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { IsString, IsOptional, ValidateNested } from "class-validator";
import { UserUpdateManyWithoutFamiliesInput } from "./UserUpdateManyWithoutFamiliesInput";
import { Type } from "class-transformer";
import { RecipeUpdateManyWithoutFamiliesInput } from "./RecipeUpdateManyWithoutFamiliesInput";
import { SmartTagUpdateManyWithoutFamiliesInput } from "./SmartTagUpdateManyWithoutFamiliesInput";

@InputType()
class FamilyUpdateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  country?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  description?: string | null;

  @ApiProperty({
    required: false,
    type: () => UserUpdateManyWithoutFamiliesInput,
  })
  @ValidateNested()
  @Type(() => UserUpdateManyWithoutFamiliesInput)
  @IsOptional()
  @Field(() => UserUpdateManyWithoutFamiliesInput, {
    nullable: true,
  })
  member?: UserUpdateManyWithoutFamiliesInput;

  @ApiProperty({
    required: false,
    type: () => UserUpdateManyWithoutFamiliesInput,
  })
  @ValidateNested()
  @Type(() => UserUpdateManyWithoutFamiliesInput)
  @IsOptional()
  @Field(() => UserUpdateManyWithoutFamiliesInput, {
    nullable: true,
  })
  ownUsers?: UserUpdateManyWithoutFamiliesInput;

  @ApiProperty({
    required: false,
    type: () => RecipeUpdateManyWithoutFamiliesInput,
  })
  @ValidateNested()
  @Type(() => RecipeUpdateManyWithoutFamiliesInput)
  @IsOptional()
  @Field(() => RecipeUpdateManyWithoutFamiliesInput, {
    nullable: true,
  })
  recipes?: RecipeUpdateManyWithoutFamiliesInput;

  @ApiProperty({
    required: false,
    type: () => SmartTagUpdateManyWithoutFamiliesInput,
  })
  @ValidateNested()
  @Type(() => SmartTagUpdateManyWithoutFamiliesInput)
  @IsOptional()
  @Field(() => SmartTagUpdateManyWithoutFamiliesInput, {
    nullable: true,
  })
  smartTags?: SmartTagUpdateManyWithoutFamiliesInput;
}

export { FamilyUpdateInput };
